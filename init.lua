...
local group = vim.api.nvim_create_augroup("WindowBufTrail", { clear = true })

-- TODO: This is almost correct. It's getting a previous buffer added to the list when going to a new tab, and when creating a split window
local function update_window_buftrail()
    local win = vim.api.nvim_get_current_win()
    local buf = vim.api.nvim_get_current_buf()

    -- only care about "normal" listed buffers
    if not vim.bo[buf].buflisted then
        return
    end

    local trail = vim.w.buftrail
    if type(trail) ~= "table" then
        trail = {}
    end

    -- append only if it’s not already the last entry
    if trail[#trail] ~= buf then
        table.insert(trail, buf)
        vim.w.buftrail = trail
    end
end

vim.api.nvim_create_autocmd({ "BufEnter", "WinEnter" }, {
    group = group,
    callback = update_window_buftrail,
})

function _G.WindowBufWinbar()
    local win    = vim.api.nvim_get_current_win()
    local curbuf = vim.api.nvim_get_current_buf()
    local trail  = vim.w.buftrail

    if type(trail) ~= "table" or #trail == 0 then
        -- fallback: just show current buffer
    ...
    for _, buf in ipairs(trail) do
        -- skip dead/unlisted buffers, avoid duplicates
        ...
        then
            ...
            if name == "" then
                ...
                -- name = vim.fn.fnamemodify(name, ":.")   -- relative path
                name = vim.fn.fnamemodify(name, ":.") -- e.g. a/b/c.lua
            end
            name = name:gsub("%%", "%%%%") -- escape %

            if buf == curbuf then
                -- highlight current buffer in this window (pick your UserN)
                table.insert(parts, "%#User4# [" .. name .. "] %*")
            else
...
-- Use that for all windows’ winbars
vim.o.winbar = "%{%v:lua.WindowBufWinbar()%}"
